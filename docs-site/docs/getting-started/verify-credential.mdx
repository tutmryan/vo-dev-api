...
- For developers, you will need a non-production instance of VO to use the Apollo Studio

## TL;DR

From the Presentation Builder area of the VO Composer:
...
From the <PortalLink path='/demo/presentation'>VO Concierge presentation demo</PortalLink>, you can also execute a presentation request and explore the data available as a result of the data sharing.
...
- export the presentation request mutation to use in API cal
...
:::note
The options to accept a revoked credential and whether to receive a callback only apply to the API call setup in the Apollo Studio, they do not impact the presentation request that will be tested in the next step of the Presentation Builder.
:::

![Presentation payload](../../static/img/presentation-payload.png)

Finally, in a non-production instance of VO, there is a ‘Try it 🚀’ button that builds the presentation request in the Apollo Studio, ready to execute API call to initiate the presentation request. The response data includes the URI of the presentation request and the base64 encoded data that represents the presentation request URI.

:::note[**Apollo Sandbox**]
We provide the <ApiLink>Apollo Sandbox</ApiLink> for non-production instances of VO, to streamline the integration and testing of verifiable credentials into your applications.
:::

![Apollo Studio presentation request example](../../static/img/apollo-sandbox-presentation-request.png)
