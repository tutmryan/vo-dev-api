...
| `vc_constraint_name`      | The claim key to apply the constraint specified by `vc_constraint_operator` and `vc_constraint_value`.                                                                                                                                               |
| `vc_constraint_operator`  | The constraint operator, either `contains`, `startsWith` or `values`.                                                                                                                                                                                |
| `vc_constraint_value`     | The constraint operator. If `values` is used, several values can be specified using a comma to separate, e.g. `123,456,789`.                                                                                                                         |
