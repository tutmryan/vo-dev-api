...
| `vc_constraint_value`     | A single constraint value, e.g. `123`, when used with `vc_constraint_operator` of either `contains` or `startsWith`.<br /><br />If the operator `values` is used, several values can be specified using a comma to separate, e.g. `123,456,789` to specify a match on either `123`, `456`, or `789`, but not `999`.<br /><br />Regardless of the `vc_constraint_operator` used, the value `oidc:login_hint` will dynamically resolve at runtime.                                                                                    |

:::tip
To match a single value in a traditional equality check way, use the `vc_constraint_operator` of `values` and specify a single value in the `vc_constraint_value`.
:::
...
const settings: OidcClientSettings = {
  ...
  scope: 'openid profile offline_access email vc_info vc_presented_attributes vc_vo_presentation vc_vo_identity api:read api:write',
}
```
...
The full list of scopes is available from the <ApiLink path="/oidc/.well-known/openid-configuration">OpenID discovery document</ApiLink>.

The Verified Orchestration OIDC provider supports standard scopes: `openid`, `profile`, `email`\* and `offline_access`\*.
...
\* The `email` scope provides a claim value only when an `email` claim is present in the verifiable credential presentation. Similarly, the `email_verified` claim is included only when an `email_verified` claim is provided.
:::
...
| `email`                                  | `email`                   | The value of the `email` claim from the verifiable credential presentation, if present.                                                                                                                                                                                                    |
| `email_verified`                         | `email`                   | A boolean indicating whether the email has been additionally verified, if present.                                                                                                                                                                                                         |
