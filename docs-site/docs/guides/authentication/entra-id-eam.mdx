...
Entra ID EAM is in public preview. Public preview features carry a higher risk of change and buggy behaviour than features that are generally available.

:::danger[Caution]

Authentications flows on Mobile devices which use Entra EAM with Verifiable Credentials (VC) are not supported.

The Microsoft Authenticator App on iOS cannot process a login and a request to present a VC at the same time.

On both iOS and Android, Microsoft applications cannot request Authenticator to present a VC during the authentication flow.

Verified Orchestration is working with Microsoft to resolve this issue.

:::

### Tenants

When using EAM with a tenant other than your home tenant, the Verified Orchestration (VO) platform must know about it. For more information, see [additional tenant IDs](/docs/guides/instance-configuration#additional-tenant-ids).
