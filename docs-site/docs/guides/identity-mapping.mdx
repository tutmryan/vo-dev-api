...
- A unique **identifier for the store**. For example in Entra ID, the Tenant ID uniquely identifies the store and is available within an Entra JWT payload as the `tid` claim.
