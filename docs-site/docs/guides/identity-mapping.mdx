...
:::tip
In the Admin UI, identity references are labelled as "Issuees".
:::
...
- A unique **identifier for the store**. For example in Entra ID, the Tenant ID identifies the store and is available within an Entra JWT payload as the `iss` claim.
- A unique **identifier for each identity** within a store. For example in Entra ID, the User Object ID available within an Entra JWT payload as the `oid` claim.
...
The table below demonstrates how an Entra identity store user can be mapped to a Verified Orchestration identity reference. This reference includes both the Entra user attributes and where those values are stored in a JWT  produced by Entra.
...
A name is a convenient way to make identity references easily recognisable without querying your organisation's identity stores. Providing the name, however, is not required. When a name is supplied, the Admin UI will use it when creating or viewing issuances, remote issuances, and presentations.
...
- **Name**: An optional human-readable name for quickly resolving the identity.

:::tip
Custom labels can be configured for each identity store's Issuer ID. This is a convenient way to make each Identity Store easier to distinguish, as when set, the Admin UI uses these human-readable names instead of the issuer ID. See the [instance configuration guide](/docs/guides/instance-configuration) for more information.
:::
...
:::tip
The Admin UI has special handling of identity references featuring `manual` as the issuer, with these being shown and label as 'Manually Issued'.
:::
...
- The Admin UI supports searching for issuances, remote issuances and presentations by linked identities.
- Admin users can find identities or list identities via the <AdminUrl path="/identities">Issuees page</AdminUrl>.

### Querying related data

Identities can be queried to return one or many by using either the Verified Orchestration identity ID or your organisation's own identity data. When using your own organisation's own identity data, you will construct and pass one or more identity references which the Verified Orchestration API will use to find the corresponding identity.
