...
VO supports adding and grouping identities through the **Identity Store** feature.
Identity mapping continues to be used to connect each credential or presentation to the correct person or entity in your organisation.

For details on setup and configuration, see the [Identity Store documentation](/docs/guides/identity-stores).

## VO identity reference

The VO platform minimises holding personal data. Instead, it stores and uses minimal identity references to link back into your organisation's own identity system.
...
VO assumes that your organisation has one or more identity stores. With each store having:
...
These two values form the unique key which is used to link a VO identity reference back to an identity within your organisation's identity stores.

### Example mapping of an Entra ID User

The table below demonstrates how an Entra ID identity store user can be mapped to a VO identity reference. This reference includes both the Entra ID user attributes and under which claims the values are stored by an Entra ID produced JWT.

| Entra ID User Attribute | JWT Payload Claim | VO Identity Field |
| ----------------------- | ----------------- | ----------------- |
...
:::info
An identity store will always be created automatically if it does not already exist for the given issuer.
If the identity store already exists, the identity will be added to it.
:::

#### Creating an identity with issuance

When creating an issuance or remote issuance, you may optionally include the identity information instead supply the VO's identity ID. This can be more convenient, as the identity is saved or updated and linked to the issuance in a single operation.
...
Identities can be queried to return one or many by using either the VO identity ID or your organisation's own identity data (issuer and identifier unique key).

The following queries are available:

- [identity](/api-reference/operations/queries/identity) finds a single identity by ID
- [identities](/api-reference/operations/queries/identities) finds multiple identities by ID
- [identityByIdentifier](/api-reference/operations/queries/identity-by-identifier) finds one identity by issuer and identifier unique key
- [identitiesByIdentifiers](/api-reference/operations/queries/identities-by-identifiers) finds one or more identities by issuer and identifier unique key
- [findIdentities](/api-reference/operations/queries/find-identities) finds identities by issuer and name partial match

You can also simply upsert and query identities in a single operation:

- [saveIdentity](/api-reference/operations/mutations/save-identity) performs an efficient upsert operation to save or update an identity by issuer and identifier unique key, data can be returned in the same operation
