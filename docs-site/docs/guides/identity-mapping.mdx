...
- A unique **identifier for the store**. For example in Entra ID, the Tenant ID uniquely identifies the store and is available within an Entra JWT payload as the `iss` claim.
- A unique **identifier for each identity** within a store. For example in Entra ID, the unique user Object ID is available within an Entra JWT payload as the `oid` claim.
...
The table below demonstrates how an Entra identity store user can be mapped to a Verified Orchestration identity reference. This reference includes both the Entra user attributes and under which claims the values are stored by an Entra produced JWT.
...
A name is a convenient way to make identity references easily recognisable without querying your organisation's identity stores. Providing the name is optional. However, when a name is supplied, the Admin UI will use it for display when creating or viewing issuances, remote issuances, and presentations.
...
:::tip
Custom labels can be configured for each identity store's issuer ID. This is a convenient way to make each identity store easier to distinguish between. When set, the Admin UI displays these human-readable labels instead of the issuer ID. See the [instance configuration guide](/docs/guides/instance-configuration) for more information.
:::
