...
Use [presentation flows](./presentation-flows.mdx) to facilitate asynchronous, link-based credential presentations for use cases such as caller verification, approvals, and data entry with verification.

Considerations:

- Easy to implement from server-side applications with no user interface.
- Suitable for workflows that require human verification, approval, or data collection.
...
- Supports optional data entry fields and custom actions (e.g. approve/reject).
...
- Orchestrating presentation flows (e.g. approvals, caller verification).
