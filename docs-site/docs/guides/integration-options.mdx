...
[Issuance](./issuance.mdx) is the process of creating and delivering a credential to the recipient's wallet (Microsoft Authenticator). The Verified Orchestration platform supports issuance in the following ways:

### In person issuance via the Admin UI

A credential administrator can issue a credential on-the-spot to a recipient using the Admin UI. The recipient must scan the QR code with their mobile device to receive the credential.

This option is available by clicking the 'Issue' button when viewing a the credential in the Admin UI, then selecting the 'In person' delivery method.
...
- Issuee identity can be selected from a linked Entra home tenant, an existing issuee identity or manually entered.
...
### Remote issuance via the Admin UI

[Remote issuance](./remote-issuance.mdx) allows you to issue credentials to recipients who are not present at the time of issuance. The recipient receives an email or SMS containing a link to receive the credential via the Portal website. An OTP (one time passcode) is used to verify the recipient.

This option is available by clicking the 'Issue' button when viewing a the credential in the Admin UI, then selecting the 'Remote' delivery method.
...
- The Portal UI handles the issuance process, including photo capture and [wallet link handling](./wallet-link-handling.mdx).

### Bulk remote issuance via the Admin UI

Bulk remote issuance allows you to issue credentials to many recipients at once. This option is available by clicking the 'Bulk issue' button when viewing a the credential in the Admin UI and uploading one or more CSV files.

### Remote issuance embedded in your applications

Same as remote issuance via the Admin UI, but programmatically via the Verified Orchestration API.
...
Use the OpenID Connect Provider to [authenticate](./authentication.mdx) by presentation of a credential for authentication and resource (API) access.

Considerations:

- Suitable for applications that need protected areas or APIs to be accessed by users holding a credential.
- Simple to implement through OIDC libraries or integration to identity providers such as Okta, Entra B2C and Entra External ID.
...
### Presentation via the Admin UI

It is possible to manually run credential presentations using the <AdminUrl path="/presentation-builder">presentation builder</AdminUrl>, however the tool's primary purpose is building and testing presentation requests.
...
Single Page or Mobile Applications will need at least a minimal secure server-side API to acquire [limited access tokens](./limited-access-tokens/) or perform other operations against the Verified Orchestration API.
...
- For applications that support authenticated home Entra tenant users, [On-Behalf-Of (OBO) authentication flow](https://learn.microsoft.com/en-us/entra/identity-platform/v2-oauth2-on-behalf-of-flow) can be used to run administrative API operations as the logged in user, for example executing revocation of all credentials for an issuee from your own off-boarding app instead of using the Verified Orchestration Admin UI.
