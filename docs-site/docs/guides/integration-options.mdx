...
This option is available by clicking the 'Issue' button when viewing a the credential in the Composer, then selecting the 'Remote' delivery method.
...
- For applications that support authenticated home Entra tenant users, [On-Behalf-Of (OBO) authentication flow](https://learn.microsoft.com/en-us/entra/identity-platform/v2-oauth2-on-behalf-of-flow) can be used to run administrative API operations as the logged in user, for example executing revocation of all credentials for an issuee from your own off-boarding app instead of using the Verified Orchestration Composer.
