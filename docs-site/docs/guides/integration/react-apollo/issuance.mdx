...
The [CreateIssuanceRequest](/docs/reference/mutations/create-issuance-request) mutation returns the issuance request details, including a URL to open in the browser or a QR code to scan.
...
The [IssuanceEvent](/docs/reference/subscriptions/issuance-event) subscription returns event data during the issuance process. The `requestStatus`` field can be used to determine the current state of the issuance:
