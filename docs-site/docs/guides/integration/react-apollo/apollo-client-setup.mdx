...
Prior to writing any React code using GraphQL via Apollo Client, you will need to set up Apollo Client to work with the VO API.

This section assumes you will be using limited access tokens from your [secure backend API](../../limited-access-tokens/secure-backend-api.mdx) to authenticate with the VO API.
...
/***
 * Gets the most recent VO Access Token response from session storage (to make it available outside of a React component e.g. the network stack).
...
/***
 * Sets the VO Access Token response in session storage (to make it available to the network stack).
 */
