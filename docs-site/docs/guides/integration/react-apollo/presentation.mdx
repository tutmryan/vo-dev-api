...
All applications that request presentations must authenticate users and provide an identity reference to obtain an issuance access token. Refer to the [Authorization guide](../authn.mdx) for further info. The React examples below run within an authenticated application.
