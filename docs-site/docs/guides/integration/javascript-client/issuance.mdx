...
All applications that issue credentials require user authentication and must provide an identity reference to the Verified Orchestration platform to track the issuance. Refer to the [Authorization guide](../authn.mdx) for further info. The [complete example below](#complete-example) uses Microsoft authorization, however there is no restriction on where identity references are sourced.
