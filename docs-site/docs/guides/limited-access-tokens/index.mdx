...
Limited access tokens support access to the VO platform API from a client application (e.g. a browser or mobile app, where an API access secret cannot be stored securely). This avoids the need to proxy API requests and callbacks through a secure back-end application.
...
The [acquireLimitedAccessToken](/api-reference/operations/mutations/acquire-limited-access-token) mutation returns an access token with specific capabilities according to the [AcquireLimitedAccessTokenInput](/api-reference/types/inputs/acquire-limited-access-token-input):
...
A sample implementation of the backend API for acquiring limited access tokens from the VO platform using [Node.JS](https://nodejs.org/en/about) is provided in the [Secure backend API guide](../limited-access-tokens/secure-backend-api.mdx).
...
- Issuance and presentations can be queried via [findIssuances](/api-reference/operations/queries/find-issuances) and [findPresentations](/api-reference/operations/queries/find-presentations) for the specified identity by `identityId`
- Issuance and presentation events can be subscribed to via [issuanceEvent](/api-reference/operations/subscriptions/issuance-event) and [presentationEvent](/api-reference/operations/subscriptions/presentation-event), using the `requestId` of the issuance or presentation request
- Issuance and presentations can be queried via [findIssuances](/api-reference/operations/queries/find-issuances) and [findPresentations](/api-reference/operations/queries/find-presentations), using the `requestId` of the issuance or presentation request
- The [contract](/api-reference/types/objects/contract) field of the [issuance](/api-reference/types/objects/issuance) and [presentation](/api-reference/types/objects/presentation) can be queried
- Contracts can be listed via [findContracts](/api-reference/operations/queries/find-contracts) with issuance and presentation data for the specified identity if the limited access token allows it
