...
Identity stores make it easy to organise, group, and manage identities in VO. Each identity store represents a source of identities, such as your organisation’s directory or a manually managed group.

## What is an identity store?

An identity store is a way to organise and group identities in VO.
...
- [Via the API](/api-reference/operations/mutations/create-identity-store)
...
3. Fill in the required details based on the store type:
   ...
   **For Entra type:**
   ...
   - **Enable authentication:** Toggle if issued tokens from this tenant should be able to authenticate with VO API.
...
- **Authentication enabled:** Use the `isAuthenticationEnabled` setting to allow tokens issued from this tenant to be authenticated with the VO APIs
...
You can create identities in VO using either Composer or the API.
...
- [API Reference](/api-reference/operations/mutations/create-identity-store)
