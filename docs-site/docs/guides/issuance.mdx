...
The issuance request specifies:
...
1. Other optional input:
   1. Values of any claims included in the credential (the claims are defined in the contract)
   1. A PIN code that the recipient must enter to install the credential (to increase security of issuance)
   1. Callback info, if the issuer wants to be notified of issuance events via a server-side endpoint)
...
```graphql title="IssuanceEvent subscription notifies when issuance is A) received and B) installed"
...
:::tip
Issuance received events can be received via the IssuanceEvent subscription and the server-side callback, however you cannot poll for issuance recevied events because they are not stored. See the [issuance samples](/docs/samples/issuance) for further details.
:::
...
Upon successful issuance, the issuing application can receive notification by any combination of:
...
:::tip
For complete code samples of different methods of receiving issuance events, see the [issuance samples](/docs/samples/issuance).
:::
