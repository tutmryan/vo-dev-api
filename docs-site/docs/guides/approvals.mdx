...
:::tip
Additional contextual information for use in the post-approval callback can be included via the [ApprovalRequestInput](/docs/reference/types/inputs/approval-request-input) `correlationId` and `requestData` fields.
:::
...
Additionally, the GitHub application can call the [actionedApprovalData](/docs/reference/api/queries/actioned-approval-data) query to obtain the actioned approval data by supplying the `approvalRequestId`.
:::
...
As an alternative to receiving a callback, applications coordinating approvals can poll for actioned approvals by calling the [actionedApprovalData](/docs/reference/api/queries/actioned-approval-data) query and supplying the `approvalRequestId`. This query returns the [ActionedApprovalData](/docs/reference/types/objects/actioned-approval-data), which is the same data the callback receives.
...
:::tip
Admin users with the [VerifiableCredential.ApprovalRequestAdmin](/docs/guides/onboarding-users#approval-request-admin) roles are able to cancel approval requests via the Verified Orchestration Admin UI.
:::
...
The Verified Orchestration Admin UI provides a <AdminUrl path="/approvals">feature</AdminUrl> for filtering, viewing and cancelling approval requests.
