...
The VO platform supports the presentation of credentials to approve (or reject) a request. This feature can be used to facilitate workflows that require approval from a human via the presentation of credentials. The approval request can include rich markdown content and a reference link to provide context to the approver.
...
1. The approval request URL will open the VO Concierge in the browser (desktop or mobile).
...
:::tip
The `callbackSecret` is a shared secret between the GitHub application and the VO platform. It is returned from the call to the `CreateApprovalRequest` mutation when creating the approval request, and can be used to verify the authenticity of the callback data.

Additionally, the GitHub application can call the [actionedApprovalData](/api-reference/operations/queries/actioned-approval-data) query to obtain the actioned approval data by supplying the `approvalRequestId`.
:::
...
:::tip
Admin users with the [VerifiableCredential.ApprovalRequestAdmin](/docs/guides/onboarding-users#approval-request-admin) roles are able to cancel approval requests via the VO Composer.
:::
...
The VO Composer provides a <AdminLink path="/approvals">feature</AdminLink> for filtering, viewing and cancelling approval requests.
