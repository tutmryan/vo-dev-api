...
VO platform creates audit entries for the following event types:
...
- Callbacks for issuances and presentations from Microsoft Entra ID Verified ID<br/>
  [sample audit entry for callback Microsoft Entra ID Verified ID](#sample-audit-entry-for-a-callback-from-microsoft-entra-id-verified-id)
