...
Note: Use this role when the identity of the presenter is known at the time of acquiring the token. If the presenter identity is not known, use the [acquire anonymous presentation token](#acquire-anonymous-presentation-token) role instead.
