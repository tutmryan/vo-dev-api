...
- The recipient receives an email or SMS containing a link to receive the credential via the Verified Orchestration Concierge website.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>Notification Method (email | sms)</td>
      <td>
    ...
    <tr>
      <td>Verification Method (email | sms)</td>
      <td>
...
:::note
"(Optional)" prefix means the column is not required and will fallback to a default value if left blank.
:::
...
Once the remote issuance is created, it can be <AdminUrl path="/remote-issuances">viewed</AdminUrl> and managed via the Composer.
