...
export async function CountIssuancesByContractQuery(
  ...
  if (criteria?.requestId) query.andWhere('issuance.request_id = :requestId', { identityId: criteria.requestId })
  if (criteria?.identityId) query.andWhere('issuance.identity_id = :identityId', { identityId: criteria.identityId })
  if (criteria?.issuedById) query.andWhere('issuance.issued_by_id = :issuedById', { issuedById: criteria.issuedById })
