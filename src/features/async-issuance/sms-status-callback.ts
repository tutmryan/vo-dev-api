...
import { smsPayloadSchema, toUserMessage, validateSmsCallbackRequest, type MessageStatuses } from '../../util/sms'
...
export const getIssuanceSmsStatusCallbackUrl = (asyncIssuanceId: string) => {
  return `${apiUrl}/external/sms-status-callback/async-issuance/issuance/${asyncIssuanceId}`
}

export const getVerificationSmsStatusCallbackUrl = (asyncIssuanceId: string) => {
  return `${apiUrl}/external/sms-status-callback/async-issuance/verification/${asyncIssuanceId}`
}
...
export async function handleSmsStatusCallback(
  ...
  const userMessage = toUserMessage(payload.messageStatus, payload.errorCode)
...
export async function addAsyncIssuanceSmsStatusEndpoint(app: Express): Promise<void> {
  app.post(
    '/external/sms-status-callback/async-issuance/:type/:asyncIssuanceId',
