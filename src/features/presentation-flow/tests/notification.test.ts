...
describe('sendPresentationFlowNotification', () => {
  beforeAfterAll()
  beforeEach(() => {
    mockedServices.clearAllMocks()
  })

  describe('sendPresentationFlowNotification function - error handling', () => {
    it('throws error for cancelled presentation flow', async () => {
      // Arrange
    ...
    it('throws error for submitted presentation flow', async () => {
      // Arrange
      const identity = await createIdentity()
      const input = await getDefaultPresentationFlowInput()
      input.identityId = identity.id
